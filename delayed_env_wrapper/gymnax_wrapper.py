...
class ConstantDelayedWrapper(GymnaxWrapper):
    ...
    def step(
        ...
        def past_init(action_buffer, action):
            action_buffer, actual_action = popleft_and_append_deque(action_buffer, action)
            n_obs, n_state, reward, done, info= self._env.step(key, state, actual_action, params)
        ...
        return jax.lax.cond((jnp.equal(buffer_size,self._delay)), past_init, pre_init,  action_buffer, action)
    @partial(jax.jit, static_argnums=(0,))
